...
def coach_change(row, data):
    ...
    try:
        ...
        next_coach = data[(data["Team"] == team) & (data["Season"] == season + 1)][
            "Coach"
        ]
...
def improve(row, column, data):
    try:
        ...
        column = column
        imp = float(
            data[(data["PlayerId"] == id) & (data["Season"] == season)][column]
        ) > float(
            data[(data["PlayerId"] == id) & (data["Season"] == season - 1)][column]
        )
        if imp:
            return "Yes"
        return "No"
...
def compare(row, column, data):
    ...
    season = int(row["Season"])
    mean = data[
        (data["Stars"] == star) & (data["Yr"] == year) & (data["Position"] == pos)
    ][stat].mean()
...
def get_player_proba(data):
    ...
    data = data.drop(
        columns=["Transfer_Portal", "PositionId", "ConferenceId", "TeamId"]
    )

    coaching_data = pd.read_csv(
        "csv/coaching_data.csv", skiprows=[0, 1], skipfooter=202
    )
    coaching_data = coaching_data.rename(columns={"FBS Team": "Team"})
    coaching_data = coaching_data[
        ["Team", "2019", "2020", "2021", "2022", "2023", "2024"]
    ]
    coaching_data = coaching_data.melt(
        id_vars="Team", var_name="Season", value_name="Coach"
    )
    ...
    for i, year in enumerate(
        ...
        file = pd.read_csv("csv/player_recruit_ranking/" + year + ".csv")
        file.rename(columns={"AthleteId": "PlayerId", "Year": "Class of"}, inplace=True)
        data = data.merge(
            file, on="PlayerId", how="left", suffixes=[None, "_" + str(i)]
        )
    ...
    if data["Position"].iloc[0] in ["DT", "DE", "DL", "DB", "CB", "S", "LB"]:
        ...
        for feat in feats:
            data[feat + "_improve"] = data.apply(
                lambda x: improve(x, feat, data), axis=1
            )
        ...
        rel_feats = [
            ...
            "SACKS_improve",
            "SOLO_improve",
            "Pct_Team_INT_improve",
            "Pct_Team_SACKS_improve",
            "Pct_Team_TFL_improve",
            "Team",
            "Conference",
            "Position",
            "Yr",
        ...
        column_transform = joblib.load("models/column_transform_defense.joblib")
        data = column_transform.transform(data)
        clf = joblib.load("models/defensive_classifier.joblib")
        data = data.toarray()
    ...
    if data["Position"].iloc[0] == "QB":
        ...
        data["Pct_Team_Pass_Completions"] = (
            data["COMPLETIONS"] / data["Team passCompletions"]
        )
        ...
        feats = [
            ...
            "YPA",
            "Pct_Team_Pass_Yds",
            "Pct_Team_Pass_Attempts",
            "Pct_Team_Pass_TDs",
            "Pct_Team_Pass_Completions",
            "Pct_Team_Ints",
        ]
        for feat in feats:
            data[feat + "_improve"] = data.apply(
                lambda x: improve_o(x, feat, data), axis=1
            )
            data[feat + "_compare"] = data.apply(
                lambda x: compare(x, feat, data), axis=1
            )
        ...
        rel_feats = [
            ...
            "Pct_Team_Pass_Completions",
            "Pct_Team_Ints",
            "Usage Overall_improve",
            "Usage Pass_improve",
            "Usage Rush_improve",
            "Usage FirstDown_improve",
            "Usage SecondDown_improve",
            "Usage ThirdDown_improve",
            "Usage StandardDowns_improve",
            "Usage PassingDowns_improve",
            "ATT_improve",
            "AVG_improve",
            "COMPLETIONS_improve",
            "INT_improve",
            ...
            "YDS_improve",
            "YPA_improve",
            "Pct_Team_Pass_Yds_improve",
            "Pct_Team_Pass_Attempts_improve",
            "Pct_Team_Pass_TDs_improve",
            "Pct_Team_Pass_Completions_improve",
            "Pct_Team_Ints_improve",
            "Usage Overall_compare",
            "Usage Pass_compare",
            "Usage Rush_compare",
            "Usage FirstDown_compare",
            "Usage SecondDown_compare",
            "Usage ThirdDown_compare",
            "Usage StandardDowns_compare",
            "Usage PassingDowns_compare",
            "ATT_compare",
            "AVG_compare",
            "COMPLETIONS_compare",
            "INT_compare",
            ...
            "YDS_compare",
            "YPA_compare",
            "Pct_Team_Pass_Yds_compare",
            "Pct_Team_Pass_Attempts_compare",
            "Pct_Team_Pass_TDs_compare",
            "Pct_Team_Pass_Completions_compare",
            "Pct_Team_Ints_compare",
            "Position",
            "Yr",
            "Stars",
            "Coach Change",
        ...
        column_transform = joblib.load("models/column_transform_qb.joblib")
        data = column_transform.transform(data)
        clf = joblib.load("models/qb_classifier.joblib")
    ...
    if data["Position"].iloc[0] == "RB":
        ...
        rel_feats = [
            ...
            "REC_improve",
            "YPR_improve",
            "Pct_Team_Rush_Yds_improve",
            "Pct_Team_Rush_Attempts_improve",
            "Pct_Team_Rush_TDs_improve",
            "Usage Overall_compare",
            "Usage Rush_compare",
            "Usage Pass_compare",
            "Usage PassingDowns_compare",
            ...
            "REC_compare",
            "YPR_compare",
            "Pct_Team_Rush_Yds_compare",
            "Pct_Team_Rush_Attempts_compare",
            "Pct_Team_Rush_TDs_compare",
            "Position",
            "Yr",
            "Stars",
            "Coach Change",
        ...
        column_transform = joblib.load("models/column_transform_rb.joblib")
        data = column_transform.transform(data)
        clf = joblib.load("models/rb_classifier.joblib")
    ...
    if data["Position"].iloc[0] in ["WR", "TE"]:
        ...
        column_transform = joblib.load("models/column_transform_wr_te.joblib")
        data = column_transform.transform(data)
        clf = joblib.load("models/wr_te_classifier.joblib")
